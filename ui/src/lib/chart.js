...
function formatChangeParts(currentValue, baselineValue) {
  ...
  return { text: `${absolute.text} / ${percent.text}`, tone };
}
...
function ensureChartChrome(canvas, theme, config) {
  ...
  shell.dataset.chartBucketSeconds = String(
  ...
  if (!shell.dataset.chartControlsBound) {
    shell.addEventListener("click", (event) => {
      ...
      const nextLookback = Number(
        button.dataset.lookbackSeconds ||
          shell.dataset.chartLookbackSeconds ||
          DEFAULT_LOOKBACK_SECONDS,
      );
      const nextResolution = Number(
        button.dataset.bucketSeconds ||
          shell.dataset.chartBucketSeconds ||
...
function buildChartData(points) {
  const dates = points.map((point) => parseSnapshotTimestamp(point.bucket_start || point.inserted_at));
  const xValues = dates.map((date) => date?.getTime() || NaN);
  const aboveValues = points.map((point) => (point.mid_above != null ? Number(point.mid_above) : null));
  const belowValues = points.map((point) => (point.mid_below != null ? Number(point.mid_below) : null));

  return {
    xValues,
    above: xValues.map((x, index) => ({ x, y: aboveValues[index] })),
    below: xValues.map((x, index) => ({ x, y: belowValues[index] })),
    baselines: {
...
function projectChartPoints(points, config, now) {
  ...
  const lastPointMs = parseSnapshotTimestamp(lastPoint.bucket_start || lastPoint.inserted_at)?.getTime();
...
function buildChartOptions(theme, chartConfig, xValues) {
  return {
    responsive: true,
    ...
    scales: {
      x: {
        ...
        afterBuildTicks(scale) {
          scale.ticks = buildDeterministicXTicks(scale.min, scale.max, chartConfig.lookbackSeconds);
        },
...
export function renderChart(canvas, contract, chart, now = new Date()) {
  ...
  let chartInstance = chartInstances.get(canvas);
  if (!chartInstance) {
    chartInstance = new Chart(canvas, {
      ...
      options: buildChartOptions(theme, chartConfig, xValues),
    });
    chartInstances.set(canvas, chartInstance);
  } else {
    ...
    chartInstance.options = buildChartOptions(theme, chartConfig, xValues);
