...
const REGION_VIEWS = new Map([["miami", DEFAULT_VIEW]]);

function regionView(contract) {
  const key = contract?.region?.trim().toLowerCase();
...
function getRegionMapTheme() {
  ...
  return {
    ...
    durationMapEase:
      Number.parseFloat(cssToken(styles, "--duration-map-ease")) || 420,
  };
}
...
function applyMinimalMapTheme(map) {
  ...
  ["water", "water-fill", "water_shadow", "waterway", "waterway-shadow"].forEach((id) => {
    setPaint(id, "fill-color", theme.water);
    setPaint(id, "line-color", theme.water);
  });

  ["background", "landcover", "landuse", "park", "land"].forEach((id) => {
...
export function initRegionMap(container, contract = null) {
  if (!container || container.dataset.mapInitialized === "true") return container?.regionMap || null;
...
export function destroyRegionMap(container) {
  if (!container) return;

  container.regionMapResizeObserver?.disconnect();
  if (container.regionMapWindowResizeHandler) {
    window.removeEventListener("resize", container.regionMapWindowResizeHandler);
  }
  container.regionMap?.remove();
  delete container.regionMap;
  delete container.regionMapResizeObserver;
  delete container.regionMapScheduleResize;
  delete container.regionMapWindowResizeHandler;
