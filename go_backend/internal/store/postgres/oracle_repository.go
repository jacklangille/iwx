...
func (r *OracleRepository) MarkResolutionPublished(ctx context.Context, eventID string, publishedAt time.Time) error {
...
func (r *OracleRepository) EnqueueOutboxEvent(ctx context.Context, event outbox.Event) error {
	_, err := r.db.ExecContext(ctx, `
...
func (r *OracleRepository) ListPendingOutboxEvents(ctx context.Context, limit int) ([]outbox.Event, error) {
	if limit <= 0 || limit > 500 {
		limit = 100
	}
...
func (r *OracleRepository) MarkOutboxEventPublished(ctx context.Context, eventID string, publishedAt time.Time) error {
	_, err := r.db.ExecContext(ctx, `
...
func (r *OracleRepository) MarkOutboxEventFailed(ctx context.Context, eventID, lastError string) error {
	_, err := r.db.ExecContext(ctx, `
		UPDATE outbox_events
